...
- **车辆配置差异** (Vehicle configuration diffs) — 规划模块，尚未实现。
- **可重复赛道场景** (Repeatable track scenarios) — 规划模块，尚未实现。
- **遥测导入格式** (Telemetry import schema) — 规划模块，尚未实现。
...
仅限自己的本地场景和手工记录。特定版本的离线模式及参数修改能力均未验证。不包含在线比赛、排名或在线货币操作，也未实现进程修改。未来测试前必须先备份存档并查看游戏规则。
